...
<nav class="bg-slate-900/95 backdrop-blur-sm fixed top-0 left-0 right-0 z-50 shadow-xl border-b border-white/10">
    <div class="navbar max-w-6xl mx-auto flex items-center px-4">
        <!-- Brand and My Page (left) -->
        <div class="flex items-center space-x-2">
            <a href="/" class="btn btn-ghost text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-300 hover:to-orange-400 transition-all duration-200">
                🎮 PokePage
            </a>
            {#if session !== null}
                <a
        ...
        <div class="hidden md:flex items-center space-x-4">
            {#if session == null}
                ...
                {/if}
            {:else}
                <div class="flex items-center bg-white/10 rounded-full px-4 py-2 border border-white/20">
                    <div class="w-2 h-2 bg-green-400 rounded-full mr-2"></div>
                    <span class="text-white text-sm font-medium">{user_email}</span>
                </div>
                {#if session !== null}
                    <a
                        href="/settings"
    ...
    {#if navOpen}
        <button
            type="button"
            class="md:hidden fixed inset-0 bg-black bg-opacity-75 z-40"
        ...
        ></button>
        <div
            class="md:hidden fixed top-0 right-0 w-80 h-full bg-slate-800/95 backdrop-blur-sm shadow-2xl z-50 flex flex-col p-6 animate-slide-in border-l border-white/20"
        >
            ...
            <div class="space-y-3">
                <a
                    href="/"
                    class="flex items-center space-x-3 p-3 rounded-lg hover:bg-white/10 transition-all duration-200 text-white"
                    on:click={closeNav}
                >
                ...
                {#if session == null}
                    {#if !isInLogInPage}
                        <button
                            aria-label="Login"
                            class="flex items-center space-x-3 p-3 rounded-lg hover:bg-white/10 transition-all duration-200 text-white w-full text-left"
                            on:click={() => {
                                closeNav();
                                goto("/login");
                            }}
                ...
                {:else}
                    ...
                    <button
                        class="flex items-center space-x-3 p-3 rounded-lg hover:bg-red-500/20 transition-all duration-200 text-red-400 w-full text-left"
                        aria-label="Logout"
                        on:click={async () => {
                            await supabase.auth.signOut();
                            closeNav();
